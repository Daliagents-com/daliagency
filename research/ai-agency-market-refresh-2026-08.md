...
Метод: 6 параллельных рисерч-агентов (верификация старого датасета, sales+креативы, кейсы+прайсинг, УТП+топ-игроки, развивающиеся ниши, Claude Code enablement), ~170 поисков и фетчей, только публичные источники.
...
| Ex-FAANG credibility | REAL только с метрик-кейсами | Fractional AI («80% hallucination reduction для Zapier») |
...
## 7. Дополнение (2026-08-02): развивающиеся ситуационные ниши

Отдельный рисерч: 2 агента, ~40 поисков.
Макро-паттерн: 2024-25 - «строить агентов», деньги 2025-26 двигаются в «поддерживать агентов работающими» (ops, evals, security, сертификация, миграции).
Тест зрелости ниши: есть ли уже листикл «top 10 agencies» - если есть, ниша созрела.

Ранжирование для агентства 5-15 человек (production AI agents):

| # | Ниша | Score | Суть и доказательства |
|---|---|---|---|
| 1 | **Agent ops / maintenance retainers** (observability + evals + drift за клиента) | 9/10 | Тулинг капитализирован (Braintrust $80M Series B @ ~$800M; Arize $70M; Langfuse куплен ClickHouse), а сервисный слой пуст - листиклов нет. Прайс-ориентиры: $500-5,000/mo; Flywheel: $20K setup + $10K/mo. Монетизирует агентов, построенных ДРУГИМИ. Чистый recurring |
| 2 | **Evals engineering as a service** | 8/10 | Parlance Labs (Hamel Husain): advisory от $128.5K/8 недель, 4,000 студентов курса из 500+ компаний = осознанный спрос. Средний тир $15-60K спринтов пуст. Вход в нишу #1 |
| 3 | **Failed-AI-pilot rescue** (PoC-to-production) | 8/10 | Оформились именованные офферы: NewGenApps «AI Rescue» (90 дней), Compoze Labs (2-нед диагностика), Virgent «AI Recovery». Топливо - «95% GenAI пилотов фейлятся» (MIT). Сливается с vibe-code rescue в одну категорию |
| 4 | **MCP server development** | 7.5/10 | $25-50K SMB MVP, $60-120K production (Intuz). Уже листиклы + оффшорное давление - компонент внутри #1-#3, не заголовок. Защищаемый слайс: enterprise security/OAuth/tenant isolation |
| 5 | **Claude Code / Codex enablement в инфре клиента** | 6.5/10 | Оффер существует и институционализируется: Anthropic Partner Network ($100M, 10K+ сертифицированных консультантов), Kriv AI на AWS Marketplace $85-235K rollout, воркшопы $15-40K, ставки $100-300/hr. Боль доказана (Microsoft отменил внутренние лицензии Claude Code, июнь 2026, из-за cost/governance). Покупатель: CTO 50-500 разработчиков. Окно: Anthropic CPN Select tier (10 сертификаций, 2 совместных прод-клиента, 1 публичный кейс) до когорты янв 2027. Риск: платформы продуктизируют setup - продавать «engineering-org AI operating model + production agents», не «установку Claude Code» |
| 6 | **Deprecation-миграции** (кампания, не ниша) | 7/10 | Жёсткие дедлайны: OpenAI Assistants API умирает 26 авг 2026, Agent Builder/Evals - 30 ноя 2026, LangChain AgentExecutor - дек 2026. Выделенных миграционных агентств НЕТ - белое пятно с 4-месячным фитилём. Лидген в ретейнер #1 |
| 7 | **AIUC-1 readiness** (страховой стандарт AI-агентов, Lloyd's-backed) | 7/10 | Квартальное тех-тестирование для удержания сертификата = встроенный recurring. Schellman - первый авторизованный аудитор (фев 2026), UiPath уже сертифицирован, prep-слой почти пуст. Red-teaming цены рынка: $8-150K, chatbot-аудиты $8-15K |
| 8 | **Agentic commerce readiness** (ACP/UCP/AP2/x402) | 6.5/10 | По-настоящему рано (<30 Shopify-мерчантов live с in-chat checkout; Netalico - первый сервисный игрок), но покупатель - e-comm, не наш профиль, и протокольный черн высокий |
| 9 | **EU AI Act readiness SMB** | 6.5/10 | Embed AI продаёт фикс-спринт EUR 9,900; mid-tier advisory EUR 50-150K; high-risk обязательства - август 2026. Но работа GRC-флейвора, credibility нет. Только как технический conformity-партнёр юрфирмы |
| 10 | Context engineering / data readiness | 6/10 | Реальная проблема (Get AI Ready - консалтинг, запущенный фев 2026 на этом тезисе), но покупается как фаза билда/rescue, не standalone: клиент ищет «почему агент галлюцинирует», а не «context engineering services» |

Убитые ниши (нет сервисного клина): AI FinOps standalone (весь momentum в тулах), shadow-AI аудиты (съедены security-вендорами и MSP), «AI-native transformation» как заголовок (занято BCG X/McKinsey вниз до Catalant), GEO/AEO (Profound - $96M Series C, $1B unicorn; экстремальная сатурация, чужой скилл-сет).

Канал, не ниша: **PE-portfolio AI value creation** (7/10) - роль «AI Operating Partner» в PE-фондах растёт, >50% mid-market portco с активными AI-инициативами; одно отношение = 10-30 клиентов. Продавать ниши #1/#3 через этот канал.

Синтез: ниши #1, #2, #5, #6, #7 - одна макро-ставка («агенты в проде ломаются и деградируют») с разными входными дверями и ОДНИМ ретейнером.
Рекомендуемая сборка: rescue/миграции/evals как входные клинья -> agent-ops retainer как аннуитет.
...
- Часть adoption-статистики раздела 7 (90% daily use, 80% enterprises с прод-агентами) - из SEO-агрегаторов, правдоподобно но не первоисточник.
